...
function PlaylistIcon({ name }) {
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(135deg, hsl(${hue},55%,22%), hsl(${hue2},60%,18%))`,
  ...
  return (
    <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
      ...
      <nav className="sidebar-nav">
        {names.length === 0 ? (
          <div style={{ padding: '12px', color: 'var(--text-dim)', fontSize: '0.85rem' }}>
            No playlists found
          </div>
        ) : (
          names.map(name => (
            <div
              key={name}
              className={`playlist-item ${name === currentPlaylist ? 'active' : ''}`}
            ...
            >
              ...
              <div className="playlist-item-info">
                ...
                <div className="playlist-item-count">
                  {playlists[name].length} track{playlists[name].length !== 1 ? 's' : ''}
                </div>
              </div>
            </div>
          ))
      ...
      <div className="sidebar-footer">
        ...
        <button className="theme-btn" onClick={onCycleTheme} title="Switch theme">
