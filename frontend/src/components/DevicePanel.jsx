export default function DevicePanel({ devices, myDeviceId, activeDeviceId, onTransfer, onClose }) {
  ...
  return (
    <>
      <div className="device-panel-backdrop" onClick={onClose} />
      <div className="device-panel">
        <div className="device-panel-header">
          <div className="device-panel-title"><DevicesIcon /> Connect to a device</div>
          <button className="btn-icon device-panel-close" onClick={onClose}><CloseIcon /></button>
        </div>
        <div className="device-panel-list">
          {devices.length === 0 ? (
            ...
            return (
              <div key={device.deviceId} className={`device-item ${isActive ? 'device-item-active' : ''}`}>
                <div className="device-item-icon"><DeviceTypeIcon type={device.deviceType} /></div>
                ...
                {!isActive && (
                  <button className="device-transfer-btn"
...
function DevicesIcon() {
  return <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M4 6h18V4H4c-1.1 0-2 .9-2 2v11H0v3h14v-3H4V6zm19 2h-6c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h6c.55 0 1-.45 1-1V9c0-.55-.45-1-1-1zm-1 9h-4v-7h4v7z"/></svg>;
}
function CloseIcon() {
  return <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M19 6.41 17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>;
}